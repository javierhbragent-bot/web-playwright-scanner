...
export async function analyzeDom(page: Page): Promise<DomSnapshot> {
  ...
  return page.evaluate(() => {
    ...
    const dialogs = Array.from(document.querySelectorAll('dialog, [role="dialog"], .modal')).map(
      (el) => {
        const titleEl = el.querySelector('[class*="title"], h1, h2, h3, [class*="header"]');
        return {
          open:
    ...
    const alerts = Array.from(
      document.querySelectorAll('[role="alert"], .alert, .notification, .toast, .message'),
    ).map((el) => ({
      text: (el.textContent ?? '').trim(),
      type: el.className.match(/alert-(\w+)|notification-(\w+)|toast-(\w+)/)?.[1] ?? undefined,
      selector: getSelector(el),
    }));
