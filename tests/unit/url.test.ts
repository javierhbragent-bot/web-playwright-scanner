...
describe('normalizeEndpointPattern', () => {
  it('replaces numeric IDs with :id', () => {
    expect(normalizeEndpointPattern('https://example.com/api/users/123')).toBe('/api/users/:id');
  });
  ...
  it('replaces MongoDB ObjectIds with :id', () => {
    expect(normalizeEndpointPattern('https://example.com/api/users/507f1f77bcf86cd799439011')).toBe(
      '/api/users/:id',
    );
  });

  it('handles multiple ID segments', () => {
    expect(normalizeEndpointPattern('https://example.com/api/users/123/posts/456')).toBe(
      '/api/users/:id/posts/:id',
    );
  });

  it('preserves non-ID segments', () => {
    expect(normalizeEndpointPattern('https://example.com/api/users/settings')).toBe(
      '/api/users/settings',
    );
  });
...
describe('matchesPattern', () => {
  ...
  it('rejects non-matching patterns', () => {
    expect(matchesPattern('https://example.com/static/file.js', '**/api/**')).toBe(false);
  });
});

describe('extractPathname', () => {
  it('extracts pathname from URL', () => {
    expect(extractPathname('https://example.com/api/users?page=1')).toBe('/api/users');
  });
});
